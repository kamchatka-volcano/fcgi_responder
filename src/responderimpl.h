...
public:
  ...
  void setErrorInfoHandler(
      std::function<void(const std::string &)> errorInfoHandler);
...
private:
  ...
  void onStdIn(uint16_t requestId,
               const StreamDataMessage<RecordType::StdIn> &msg);
...
private:
  ...
  using ResponseSender =
      std::function<void(uint16_t, std::string &&, std::string &&)>;
  std::shared_ptr<ResponseSender> responseSender_;

private:
  template <typename TMsg> void sendMessage(uint16_t requestId, TMsg &&msg);
};
