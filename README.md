...
To build `fcgi_responder_fuzzer` for debugging input data run the following command:
```
cd fcgi_responder
...
To build `fcgi_responder_fuzzer` for running fuzzing tests with `afl-fuzz` utility run the following command:
```
cd fcgi_responder
...
Results can be found in `fuzz_tests/res` directory.

To learn more about fuzzing testing, check out [AFL++ docs](https://github.com/AFLplusplus/AFLplusplus/tree/stable/docs) and this [tutorial](https://github.com/antonio-morales/Fuzzing101/).

Input data inside `/fuzz_tests/input` is generated by `fuzz_input_generator` utility. It's a modified version of unit tests, writing the input data to files. To build it, use the following commands:
```
cd fcgi_responder
